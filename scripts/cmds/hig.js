...
module.exports = {
  ...
  onStart: async function ({ event, api, usersData }) {
    ...
    // Hugger's avatar positioning (X = 285, Y = 110)
    ctx.save();
    ctx.beginPath();
    ctx.arc(285, 110, 50, 0, Math.PI * 2);  // Circle center X = 285, Y = 110
    ctx.closePath();
    ctx.clip();
    ctx.drawImage(huggerAvatar, 235, 60, 100, 100);  // Image X = 235 (shifted to the right)
    ctx.restore();

    // Target's avatar positioning (X = 460, Y = 160)
    ctx.save();
    ctx.beginPath();
    ctx.arc(460, 160, 50, 0, Math.PI * 2);  // Circle center X = 460
    ctx.closePath();
    ctx.clip();
    ctx.drawImage(targetAvatar, 410, 110, 100, 100);  // Image X = 410
    ctx.restore();
